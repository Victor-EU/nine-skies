...
| A section's ground is not what the pipeline would produce | The gate on a machine with a world reports drift between the section and the corridor | Re-cut. The comparison only runs where both exist, which is the machine that can fix it; CI takes the section on trust, so the honest statement of the gap is that CI verifies a section against its *route*, and the pipeline's golden probes are what verify the elevations (F24) |
...
- [ ] **Commit the PROJ anchor table so the projection check runs on a fresh clone** (D21) — `projectAlbers` is a second implementation of the pipeline's projection and `test/route/albers.test.ts` is what stops the two drifting, but it needs PROJ's own answers out of a built manifest, so it is three of the ten tests still skipped without a world. Every committed section depends on that projection, and a section cannot defend it: a projection change reads as a stale section rather than as a wrong projection (F24)
- [ ] **Teach CI that a section's numbers came from the pipeline** (D21) — today CI verifies that a section matches its route, not that its ground is real; a hand-edited array passes. Only a machine with a world can re-derive it, which is where the comparison runs. Probably a signature over the cut rather than more checking, and not urgent until more than one person authors routes (F24)
