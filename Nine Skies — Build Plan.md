...
- [ ] **Teach CI that a section's numbers came from the pipeline** (D21) — today CI verifies that a section matches its route, not that its ground is real; a hand-edited array passes. Only a machine with a world can re-derive it, which is where that comparison runs. D22 shows the shape of the fix and also why it does not transfer: CI re-derives the projection table because PROJ is a wheel, and cannot re-derive a section because a corridor is 14 GB of rasters. Probably a signature over the cut. Not urgent while one person authors routes, but it is now the only committed number in the repository taken on trust (F24, F25)
