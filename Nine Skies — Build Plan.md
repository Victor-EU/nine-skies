...
| Input | Keyboard+mouse and gamepad behind one abstraction from day one — **built** (F33): one binding table, keyboard and gamepad sources reading it, the help block and the `preventDefault` set derived from it, both devices live at once. Mouse look is not in it yet; the pad path is tested but has not been held | 2 |
...
- [ ] **Comfort pass before the cohort, not after G1** — the critical path already says to start it early and the risk register moves it up on the first participant who stops for discomfort. F33 adds a reason: the roll lag is the one heaviness cue the cohort cannot see, because the camera never banks. FOV, camera smoothing and a horizon-lock option are the GDD's list; the smoothing has to live outside `placeAt`, which the frame-cost capture relies on being a function of position alone (D25)
