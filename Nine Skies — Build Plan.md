...
The compression ratio is deliberately **not** in this protocol. It was, and it could not have produced a result: with A held the three candidates are the same flight frame for frame (F15), so a cohort ranking them would have ranked noise and the gate would have recorded a decision it never made. The trip-length comparison the GDD asks for in compression's name — 40 minutes against 17 — is a **cruise speed** question, and twelve minutes of a twenty-five-minute route is not enough of one to judge it; it moves to G2, where a whole leg is flown.
...
- [ ] **Pacing — how fast should cruise be?** The question the GDD asked in compression's name. `MODE_GROUND_KM_PER_MIN` ∈ {80, 130, 190} puts Sea to Sky at 40.2 / 24.8 / 16.9 minutes, which is the spread the GDD wanted to compare (F15). Needs a toggle and a whole leg to judge, so it is a G2 question. Working bounds are the GDD's: 25 minutes the ceiling for a narrated trip, 15 the floor.
...
- [ ] Add the cruise-speed toggle for the pacing question ({80, 130, 190} km/min) — half a day, and it is the axis the GDD actually wanted compared
