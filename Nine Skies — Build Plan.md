...
| D6 | **Horizontal compression and vertical exaggeration are runtime uniforms, never baked into data** | The 1:5 / 1:8 / 1:12 test is a playtest, and a playtest needs a live toggle, not three data builds |
...
| **1 — Prototype** | 6 | Corridor terrain streaming, LOD, flight model, density and temperature, minimal HUD, one palette gradient, live compression toggle | **G1** |
...
**G1 — does 3D earn its cost?** *(the GDD's go/no-go)* Ten players with no flight-sim experience fly the corridor for twelve minutes in each of 1:5, 1:8 and 1:12, order randomised. Pass requires: **≥ 6 of 10 remark on the climb, the thin air or the plane going heavy without being prompted**; median boredom onset falls *after* the plateau edge rather than before; and one compression ratio wins the preference ranking clearly. A fail triggers the Godot evaluation, two weeks budgeted; a muddy result extends the prototype by two weeks rather than proceeding.
