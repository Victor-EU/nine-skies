...
| Expedition integrity | Automated autopilot replay of all nine routes; assert every beat fires exactly once and in order | Nightly |
...
- [ ] **Pacing — how fast should cruise be?** The question the GDD asked in compression's name, and the axis that moves the exchange rate between distance and altitude. Built and keyed to `P`; `CRUISE_CANDIDATES` ∈ {80, 130, 190} puts Sea to Sky at 40.2 / 24.8 / 16.9 minutes, the spread the GDD wanted to compare (F15). **The top of that range does not fly**: above 135 km/min the aircraft arrives at Lhasa below the plateau rim and Expedition 1 cannot be completed, so the window where the climb budget and the GDD's 25-minute narrative ceiling both hold is 129–135 km/min and the shipped 130 is inside it by a kilometre a minute (F16). Decide at G2, after choosing between a per-leg speed profile, more climb rate, or a longer narrated trip — 190 is not a preference question until one of those lands.
...
- [ ] Decide how the top of the pacing range opens — per-leg speed profile, more climb rate, or a narrated trip longer than 25 minutes. Blocks the G2 pacing A/B, not G1
