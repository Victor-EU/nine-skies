...
| Layer | Method | Runs |
| --- | --- | --- |
| Pipeline | Six golden elevation/hydrology probes, plus a checksum on the tile manifest. Where a route flies over a probe's coordinates, that probe also runs against the committed section — with no world, in CI (D23) | Every pipeline commit |
...
| A source raster is intact but wrong | Nothing catches it today — `acquire.py` checks byte count, not content | The open action above: a digest per tile at fetch time. Until then the golden probes are the backstop, which is what they are for, and they cover named places rather than coverage (F26) |
...
- [ ] **Record a digest per source raster at fetch time** — `acquire.py` verifies a download by its byte count and nothing else, so "the corridor was built from the tiles the mirror served" is the one link in the chain from published raster to committed elevation that still cannot be checked after the fact. Cheap, and worth doing before phase 2 turns 331 tiles into several thousand; whether the digests can be checked against something the mirror publishes rather than only against our own first download is the part that needs looking up (F26)
