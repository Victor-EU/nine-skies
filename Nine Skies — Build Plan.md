...
| An authored route turns out not to be flyable | The clearance replay (D17) fails for any route, or is skipped because no corridor is built | Fix the route, not the check. The replay runs on every route at authoring time — `npm run content:validate`, or `make routes` to make a skip an exit code — and a route that has never been flown over real ground is not authored yet. CI has no corridor until G2, so it prints NOT CHECKED rather than a green tick |
...
| An expedition wants to land and the gate cannot tell it whether it can | Any expedition authors an arrival at or below the clearance it keeps; today the schema rejects it at parse time (F22) | The floor taper (D20). Until it lands, an expedition authors a flypast height or authors none and reads the measured one off the gate. The failure is loud and at parse time rather than silent, which is the part that was worth buying early |
...
- [ ] **Taper the altitude floor to the arrival height** (D20, F22) — the gate validates a flypast and cannot validate a landing, which is the arrival eight of the nine expeditions want. The model is clear and the number in it wants measuring, the way F20's margin and F21's stride each did
- [ ] **Make the corridor a CI artefact so the route gate stops being skippable** (G2, D17/D19) — today the schema half runs on every commit and the route half runs on the author's machine via `make routes --require-world`. The risk register already lists a skipped clearance check as a failure rather than a pass; this is what makes that true in CI
