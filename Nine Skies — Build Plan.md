...
| Expedition runner | Route polyline, per-leg speed mode, autopilot with hand-off and rejoin, **location-triggered beats** so detours cannot desync narration, resume-at-last-beat | 5 |
...
**G2 is blocked before it is scheduled.** Its protocol flies one complete Expedition 1, and at the shipped pacing the aircraft does not complete it — it is inside a mountain at the midpoint (F17). One of the four remedies has to land before a cohort is booked, and the choice between them changes what G2 measures: a speed profile makes the trip thirty-eight minutes, which is itself the thing the gate's second criterion is asking about.
...
| Expedition integrity | Automated autopilot replay of all nine routes: assert the aircraft is never lower than the ground (D17, built in phase 1 for Sea to Sky), and that every beat fires exactly once and in order | Nightly |
...
- [ ] **Pacing — how fast should cruise be?** The question the GDD asked in compression's name, and the axis that moves the exchange rate between distance and altitude. Built and keyed to `P`; `CRUISE_CANDIDATES` ∈ {80, 130, 190} (F15). **None of the three flies Expedition 1.** Measured against the ground rather than against the destination, Sea to Sky clears at 92 km/min and not above it, and at 92 the flight takes 28.5 minutes against the GDD's 25-minute narrative ceiling — so the feasible window is empty, not narrow, and the shipped 130 flies into the Hengduan at 1,954 km, thirteen minutes in (F17). This is no longer a preference question that a cohort can settle. It becomes one again only after the route is made flyable: per-leg speed profile (works today, 38-minute trip), more climb rate, a reroute needing the 90 m hero grid, or a longer narrated trip. **Decide before G2, which cannot run until then.**
...
- [ ] **Decide how Expedition 1 is made flyable** — per-leg speed profile, more climb rate, reroute (needs the hero grid), or a longer narrated trip. This is the pacing decision now; it blocks G2 and the route authoring behind it, and it is a design call rather than an engineering one
