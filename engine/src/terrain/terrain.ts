...
export interface TerrainStats {
   ...
   * the palette, and a still or a measurement waits for this (F87). The fine
   * images near the camera are counted too (F91).
   */
...
interface LodBucket {
  ...
  /**
   * The near relief's layer + 1 for each of the instance's sub-tiles, 0
   * where one has none on the GPU (F94): a row of them, south to north, to a
...
function colourLattice(cover: HeroCover): string {
...
class TileLattice implements DrawnTiles {
  ...
  reliefDrawn = 0;
  /** The country's relief at the source's spacing, over the sub-tiles nearest the camera (F94). */
  ...
  constructor(
    ...
    this.fine =
      fine && colour
        ? new FineColour(fine.reach, fine.source, (texture) => {
    ...
    this.near =
      near && this.relief
        ? new FineColour(
            ...
            near.source,
            (texture) => {
              this.material.uniforms.uReliefNear!.value = texture;
            },
            true,
          )
        : null;
    ...
    this.material = createTerrainMaterial(this.heights.texture, {
      ...
      ...(this.relief && {
        relief: {
          ...
          ...(this.near && {
            near: {
              texture: this.near.texture,
              samples: this.near.source.samples,
              fadeM: [this.near.reach.fullM, this.near.reach.goneM] as const,
            },
          }),
        },
      }),
  ...
  beginFrame(cameraEastM: number, cameraNorthM: number): void {
  ...
  private distanceM(i: number, j: number, sideM: number): number {
    const dx = Math.max(i * sideM - this.cameraEastM, 0, this.cameraEastM - (i + 1) * sideM);
    const dy = Math.max(j * sideM - this.cameraNorthM, 0, this.cameraNorthM - (j + 1) * sideM);
  ...
  setScale(scale: WorldScale, hazeDensityPerM: number): void {
    ...
    if (this.fine) {
      (u.uFineFade!.value as { set(x: number, y: number): void }).set(
        this.fine.reach.fullM / scale.horizontalCompression,
    ...
    if (this.near) {
      (u.uReliefNearFade!.value as { set(x: number, y: number): void }).set(
        this.near.reach.fullM / scale.horizontalCompression,
...
export class Terrain {
  ...
  update(eastM: number, northM: number, altitudeM: number): Vector3 {
    ...
    for (const lattice of this.lattices) lattice.beginFrame(eastM, northM);
    let missing = 0;
