 ...
 * `test/terrain/tileCodec.test.ts`, which decodes a file the pipeline wrote.
 */
 ...
 */
export function undelta(d: Int16Array, samples: number): Int16Array {
  if (d.length !== samples * samples) {
    throw new Error(`${d.length} deltas for a ${samples} x ${samples} tile`);
  }
  ...
  for (let r = 0; r < samples; r++) {
    ...
    first += d[row]!;
    out[row] = first;
    for (let c = 1; c < samples; c++) out[row + c] = out[row + c - 1]! + d[row + c]!;
  }
  return out;
}

/** The encoder's first two steps, for tests and for nothing else. */
export function deltaPlanes(tile: Int16Array, samples: number): Uint8Array {
  const d = new Int16Array(tile.length);
  for (let r = 0; r < samples; r++) {
    const row = r * samples;
    d[row] = r === 0 ? tile[0]! : tile[row]! - tile[row - samples]!;
    for (let c = 1; c < samples; c++) d[row + c] = tile[row + c]! - tile[row + c - 1]!;
  }
  const n = d.length;
...
async function gunzip(bytes: Uint8Array): Promise<Uint8Array> {
  ...
  // shared buffer is not one, and the copy is 4 kB.
...
export async function decodeTile(bytes: Uint8Array, samples: number): Promise<Int16Array> {
  const planesBytes = samples * samples * 2;
  if (bytes.length >= 2 && bytes[0] === GZIP_MAGIC_0 && bytes[1] === GZIP_MAGIC_1) {
    ...
    if (planes && planes.length === planesBytes) return undelta(unplanes(planes), samples);
  }
  if (bytes.length === planesBytes) return undelta(unplanes(bytes), samples);
  throw new Error(`a ${bytes.length}-byte file is not a ${samples}-sample tile in ${TILE_CODEC}`);
}
