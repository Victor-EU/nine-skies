...
import { DEFAULT_COMFORT, TEXT_SCALE_CANDIDATES, textScaleLabel } from "../../engine/src/gfx/comfort.js";
...
describe("the conversions", () => {
  ...
  it("names both systems and defaults to the one the build is written in", () => {
    ...
    expect(DEFAULT_COMFORT.units).toBe("metric");
...
describe("the text size", () => {
  it("is a cycle with the designed size at the bottom of it", () => {
    expect(TEXT_SCALE_CANDIDATES[0]).toBe(1);
    expect(DEFAULT_COMFORT.textScale).toBe(1);
    expect(TEXT_SCALE_CANDIDATES[TEXT_SCALE_CANDIDATES.length - 1]).toBe(1.5);
    expect(textScaleLabel(1.25)).toBe("125 %");
  });
});
