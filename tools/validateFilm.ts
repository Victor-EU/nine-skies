 ...
 * finished nine scenes, which is the launch setting. Needs no world; when
 * one is built it is asked which hero grids exist.
 */
import { existsSync } from "node:fs";
import { TEXT_LINE_BUDGET, textLines } from "../content/scenes.ts";
import { SCENE_S } from "../engine/src/film/timeline.js";
import { formatProblems, heroBuiltIn, loadFilm } from "./film.ts";
...
const { film, problems } = loadFilm(undefined, { complete, heroBuilt: heroBuiltIn(worldDir) });
